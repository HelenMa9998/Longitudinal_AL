...
def plot_embedding(X, query_sample, target_num, title):
    ...
    plt.savefig("picture/" + str(title) + ".png", dpi=1500)
    # plt.legend((s5),('selected sample'))

    plt.legend((s1,s2,s3,s4,s5),('target == 00','target <= 50','target <= 100','target <=200','target > 200','selected sample'))
...
def visualiazation(X_train_A, X_train_B, query_sample, target_num, strategy_name, rd):
    
    X_train = torch.tensor((X_train_A.reshape(X_train_A.shape[0], 256*256).astype('float32')) / 255.0)
    X_tsne = tSNE(X_train)

    plot_embedding(X_tsne, query_sample, target_num, f"tSNE-{strategy_name}-{rd}")
    print("finished!!!")
