...
class HybridSampling(Strategy):
    ...
    def calculate_uncertainty(self, AL_method, seed):
        unlabeled_idxs, unlabeled_data = self.dataset.get_unlabeled_data()
        probs = self.predict_prob_dropout_split(unlabeled_data, AL_method, seed, n_drop=self.n_drop)# 10 times summation
    ...
    def query(self, n, param3, weight_maps, AL_method, seed):
        ...
        uncertainty_scores = self.calculate_uncertainty(AL_method, seed)
        # You might want to determine a threshold or a subset size for the uncertain samples
        uncertain_idxs = unlabeled_idxs[uncertainty_scores.sort()[1][:500]]
        uncertain_data = unlabeled_data[uncertainty_scores.sort()[1][:500]]
